  ...
  module UnixDaemon
    ...
    def platform_daemon (&block)
      safefork do
        ...
        STDOUT.reopen("#{RAILS_ROOT}/log/yarb_server.out", "a")
    ...
    def stop
      pid = read_pid_file
      raise "yarb_server doesn't appear to be running" unless pid
      $stdout.puts("shutting down yarb server...")
      Process.kill("TERM", pid)
      60.times { Process.kill(0, pid); sleep(2); $stdout.print '.' }
    ...
    def write_pid_file
      raise "yarb_server may already be running, a pid file exists: #{@cfg.pid_file}" if read_pid_file
